...
(function (root, factory) {
  ...
  function fileWorkflow(c) {
    ...
    return [
      ...
      "      - name: Upload Android installable builds",
      "        uses: actions/upload-artifact@v4",
      "        with:",
      "          name: android-installable-builds",
      "          path: |",
      ...
      "          BUNDLE_ID=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' \"$APP_PATH/Info.plist\" 2>/dev/null || true)",
      "          if [ -z \"$BUNDLE_ID\" ] || [[ \"$BUNDLE_ID\" == *'$('* ]]; then BUNDLE_ID=\"" + c.appId + "\"; fi",
      "          rm -rf ../nativize-downloads/ios-simulator-preview",
      "          mkdir -p ../nativize-downloads/ios-simulator-preview",
      "          mkdir -p ../nativize-downloads/ios-simulator-preview/payload",
      "          ditto \"$APP_PATH\" \"../nativize-downloads/ios-simulator-preview/payload/$(basename \"$APP_PATH\")\"",
      "          (cd ../nativize-downloads/ios-simulator-preview/payload && /usr/bin/tar -czf ../ios-simulator-preview-app.tar.gz \"$(basename \"$APP_PATH\")\")",
      "          rm -rf ../nativize-downloads/ios-simulator-preview/payload",
      "          cat > ../nativize-downloads/ios-simulator-preview/README.txt <<'EOF'",
      ...
      "          2. Open install-in-simulator.txt.",
      "          3. Paste the commands into Terminal. If Terminal is not in this folder,",
      "             the commands will look for the newest preview in Downloads/Desktop.",
      "          EOF",
      "          cat > ../nativize-downloads/ios-simulator-preview/install-in-simulator.txt <<'EOF'",
      "          # Paste these commands into Terminal. They work from this folder,",
      "          # or they will find the newest Nativize preview in Downloads/Desktop.",
      "          set -euo pipefail",
      "          HERE=\"$(pwd)\"",
      ...
      "          path: ios/nativize-downloads/ios-simulator-preview/**",
      "          if-no-files-found: error",
