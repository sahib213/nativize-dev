...
(function (root, factory) {
  ...
  function normalizeConfig(input) {
    ...
    var appName = String(gated.appName || "My Lovable App").trim();
    var platforms = Array.isArray(gated.platforms) && gated.platforms.length ? gated.platforms.slice() : DEFAULT_PLATFORMS.slice();
    var cfg = {
      appName: appName,
      appId: normalizeAppId(gated.appId, appName),
      webDir: normalizeWebDir(gated.webDir),
      githubRepo: String(gated.githubRepo || "").trim(), // "owner/repo"
  ...
  function normalizeSocialAuth(input) {
    ...
    SOCIAL_AUTH_CATALOG.forEach(function (prov) {
      ...
      prov.fields.forEach(function (f) {
        values[f.key] = String(raw[f.key] == null ? "" : raw[f.key]).trim();
      });
