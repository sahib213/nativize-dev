...
(function (root, factory) {
  ...
  function fileWorkflow(c) {
    ...
    return [
      ...
      "          xcrun simctl launch \"$DEVICE\" " + c.appId,
      "          xcrun simctl terminate \"$DEVICE\" " + c.appId + " || true",
      ...
      "          ditto \"$APP_PATH\" \"../nativize-downloads/ios-simulator-preview/$(basename \"$APP_PATH\")\"",
      "          cat > ../nativize-downloads/ios-simulator-preview/README.txt <<'EOF'",
      "          This is an iOS Simulator preview, not a Mac app.",
      "          Do not double-click the .app bundle. macOS will say it is unsupported.",
      "          Install Xcode, unzip this folder, then run:",
      "          ./install-in-simulator.sh",
      "          EOF",
      "          cat > ../nativize-downloads/ios-simulator-preview/install-in-simulator.sh <<'EOF'",
      "          #!/usr/bin/env bash",
      "          set -euo pipefail",
      "          HERE=\"$(cd \"$(dirname \"$0\")\" && pwd)\"",
      "          APP_PATH=\"$(find \"$HERE\" -maxdepth 1 -name '*.app' -type d | head -n 1)\"",
      "          if [ ! -d \"$APP_PATH\" ]; then echo 'No iOS Simulator .app found next to this script.'; exit 1; fi",
      "          DEVICE=\"${1:-iPhone 16}\"",
      ...
      "          xcrun simctl launch \"$DEVICE\" " + c.appId,
      "          EOF",
      "          chmod +x ../nativize-downloads/ios-simulator-preview/install-in-simulator.sh",
