...
(function (root, factory) {
  ...
  function normalizeConfig(input) {
    ...
    var cfg = {
      ...
      // Real Firebase config the user pasted/selected in the panel; when present
      // it replaces the non-working placeholders so push actually works.
      firebaseAndroidJson: gated.enablePush === true ? boundedText(gated.firebaseAndroidJson || "", 200000, "google-services.json") : "",
      firebaseIosPlist: gated.enablePush === true ? boundedText(gated.firebaseIosPlist || "", 200000, "GoogleService-Info.plist") : "",
  ...
  function generateKit(input) {
    ...
    if (c.enablePush) {
      ...
      // Bake the REAL Firebase config in when provided. nativize-push-config.sh
      // only writes placeholders for files that don't already exist, so shipping
      // these means the build uses the user's real project → push actually works.
      if (c.firebaseAndroidJson) files["android/app/google-services.json"] = c.firebaseAndroidJson;
      if (c.firebaseIosPlist) files["ios/App/App/GoogleService-Info.plist"] = c.firebaseIosPlist;
    }
