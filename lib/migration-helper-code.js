...
(function (root, factory) {
  ...
  var LINES = [
    ...
    "    }",
    "    if (action === \"storage_object_chunk\") {",
    "      const bucket = String(body.bucket || \"\");",
    "      const name = String(body.name || \"\");",
    "      const offset = Math.max(0, Number(body.offset) || 0);",
    "      const limit = Math.min(1000000, Math.max(1, Number(body.limit) || 1000000));",
    "      if (!bucket || !name) return json({ error: \"bad object\" }, 400);",
    "      if (!SUPABASE_URL || !SERVICE_KEY) return json({ error: \"storage credentials unavailable\" }, 500);",
    "      const url = SUPABASE_URL + \"/storage/v1/object/\" + encodeURIComponent(bucket) + \"/\" + name.split(\"/\").map(encodeURIComponent).join(\"/\");",
    "      const res = await fetch(url, { headers: { Authorization: \"Bearer \" + SERVICE_KEY, Range: \"bytes=\" + offset + \"-\" + (offset + limit - 1) } });",
    "      if (!res.ok && res.status !== 206) return json({ error: \"download failed \" + res.status }, 502);",
    "      const buf = new Uint8Array(await res.arrayBuffer());",
    "      let bin = \"\"; for (let i = 0; i < buf.length; i++) bin += String.fromCharCode(buf[i]);",
    "      const range = res.headers.get(\"content-range\") || \"\";",
    "      const m = range.match(/bytes (\\d+)-(\\d+)\\/(\\d+)/);",
    "      const total = m ? Number(m[3]) : (offset + buf.length);",
    "      const next = offset + buf.length;",
    "      return json({ contentType: res.headers.get(\"content-type\") || \"application/octet-stream\", base64: btoa(bin), size: buf.length, total: total, next: next, done: next >= total });",
    "    }",
    "    if (action === \"storage_signed_url\") {",
    "      const bucket = String(body.bucket || \"\");",
    "      const name = String(body.name || \"\");",
    "      if (!bucket || !name) return json({ error: \"bad object\" }, 400);",
    "      if (!SUPABASE_URL || !SERVICE_KEY) return json({ error: \"storage credentials unavailable\" }, 500);",
    "      const path = encodeURIComponent(bucket) + \"/\" + name.split(\"/\").map(encodeURIComponent).join(\"/\");",
    "      const res = await fetch(SUPABASE_URL + \"/storage/v1/object/sign/\" + path, { method: \"POST\", headers: { Authorization: \"Bearer \" + SERVICE_KEY, apikey: SERVICE_KEY, \"Content-Type\": \"application/json\" }, body: JSON.stringify({ expiresIn: 3600 }) });",
    "      const data = await res.json().catch(() => ({}));",
    "      if (!res.ok) return json({ error: \"sign failed \" + res.status }, 502);",
    "      const signed = data.signedURL || data.signedUrl || data.signed_url || \"\";",
    "      const url = String(signed).startsWith(\"http\") ? signed : SUPABASE_URL + \"/storage/v1\" + signed;",
    "      return json({ url: url, expiresIn: 3600 });",
    "    }",
    ...
    "      return new Response(res.body, { status: 200, headers: { ...cors, \"Content-Type\": res.headers.get(\"content-type\") || \"application/octet-stream\", \"x-nativize-size\": res.headers.get(\"content-length\") || \"0\" } });",
    "    }",
    ...
    "  const exts = await sql`select extname from pg_extension where extname in ('vector','pgcrypto','uuid-ossp','citext','pg_trgm','unaccent') order by extname`;",
    "  const extensions = exts.map((e) => \"create extension if not exists \" + ident(e.extname) + \" with schema extensions;\");",
    ...
    "  const cols = await sql`select c.relname as table_name, a.attname as column_name, pg_catalog.format_type(a.atttypid, a.atttypmod) as formatted_type, a.attnotnull, pg_get_expr(d.adbin, d.adrelid) as column_default from pg_class c join pg_namespace n on n.oid=c.relnamespace join pg_attribute a on a.attrelid=c.oid left join pg_attrdef d on d.adrelid=c.oid and d.adnum=a.attnum where n.nspname='public' and c.relkind in ('r','p') and a.attnum > 0 and not a.attisdropped order by c.relname, a.attnum`;",
    ...
    "      let d = \"  \" + ident(c.column_name) + \" \" + c.formatted_type;",
    ...
    "  return { extensions, types, tables, constraints, indexes, rls, sequences, tableNames };",
    "}",
    ""
  ];
